...
try
    ...
    PsychDebugWindowConfiguration(0,1); % AE: can make the screen transparent to facilitate debugging
    ...
    imgList = {'center.bmp' 'mid_NE.bmp' 'mid_NW.bmp' 'mid_SE.bmp' 'mid_SW.bmp' 'outer_NE.bmp' 'outer_NW.bmp' 'outer_SE.bmp' 'outer_SW.bmp'};   
    ...
    for i = 1:length(imgList)
        ...
        %AE: Moved from botton so that I can register file name early when
        %trial starts
        ...
        %AE:moved below two lines to begining of for loop so that I can
        %register file names to edf file right after trial starts
        %imgName = char(imgList(i)); % Get image file name for current trial
        %imgInfo = imfinfo(imgName); % Get image file info
...
% AE: moved this section to bottom of STEP 5.3 to use the stimulus as cue and
...
%         WaitSecs(0.1); % Allow some time to record a few samples before presenting first stimulus
        ...
        Eyelink('SetOfflineMode');% Put tracker in idle/offline mode before recording
        Eyelink('StartRecording'); % Start tracker recording
        WaitSecs(0.5); % Allow some time to record a few samples before presenting first stimulus
        ...
        while 1 % loop until error, space bar or button press
            ...
            %AE: Added this section below to end trial without pressing keys
            %Note this takes away button press functionality to end trial
            %End trial after 4 seconds
            WaitSecs(5);
            %Write message to EDF file to mark the timeout
            Eyelink('Message', '!V TRIAL_VAR timeout %s', 'yes');
            reactionTime = round((GetSecs-RtStart)*1000); % Calculate RT from stimulus onset
            break; % Exit while loop
            % end of added section by AE %%%%%%%%%%%%%%%%%%%%%%%%%
